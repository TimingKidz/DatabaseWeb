...
class DataController extends Controller
{
    ...
    public function addstockin(Request $request)
    {
        try {
            ...
            foreach ($request->dataarr as $key => $item) {
                ...
                $qty = $item["qty"];
                $bPrice = $item["p"];
                DB::insert("insert into stockinDetails(stockinNumber,productCode,quantityOrdered,buyPrice)
                values ('$stockinNumber', '$proCode', '$qty','$bPrice')");
    ...
    public function addproduct(Request $request)
    {
        try {
            ...
            VALUES ('$request->code','$request->name','$request->line','$request->scale','$request->vender','$request->des','0','0','$request->msrp')");
            return json_encode("Success");
        } catch (Exception $e) {
            return json_encode($e->getMessage());
        }
    ...
    public function ermReq(Request $request)
    {
        ...
        foreach (job::$Title as &$value) {
            if ($value[1] < job::$Match[$code]) {
                array_push($jobEm, $value);
            }
        }
        $jobmm = array();
        $jobnm = array();
        $jobm = array();
        foreach ($data as &$value) {
            if (job::$Match[$value->jobTitle] <= job::$Match[$code] and job::$Match[$value->jobTitle] > 0) {
                array_push($jobmm, $value);
            }
            if ("$id" == $value->reportsTo) {
                array_push($jobnm, $value);
            }
            if ("$rep" == $value->employeeNumber) {
                array_push($jobm, $value);
            }
        }
    ...
    public function test()
    {
        ...
        foreach ($data as &$value) {
            ...
            print_r("<br>");
            if (job::$Match[$value->jobTitle] <= job::$Match[$vp[1]] and job::$Match[$value->jobTitle] > 0) {
                array_push($jobmm, $value);
            }
            if ("1056" == $value->reportsTo) {
                array_push($jobnm, $value);
            }
        }
